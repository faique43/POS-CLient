...
exports.createProduct = async (req, res) => {
  ...
  try {
    ...
    // reduce all the inventoryused in the product
    const inventoryArray = [];
    for (let item in inventoryUsed) {
      const inventory = await Inventory.findById(inventoryUsed[item].item);
      inventory.quantity -= inventoryUsed[item].quantity;
      inventoryArray.push(inventory);
      // if the inventory becomes less than 0 dont allow
